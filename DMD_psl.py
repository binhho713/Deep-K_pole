...
generator.load_state_dict(torch.load(f"models/G_ts_psl_linear-{model_ver}.model", weights_only=True))
generator.eval()
...
pred_path = f"./models/pred_DMD_psl_{k_query}_"
sep_path = f"./models/sep_DMD_psl_{k_query}_"

check_freq = 100
save_freq = 20
lr = 0.00001

logger = setup_logger(f'DMD_psl_train_{k_query}_logger', f'DMD_psl_training_{k_query}.log')
...
# ----------
    ...
    # --- load & normalize ---
    inTs = np.load('base_ts_171_psl.npy', allow_pickle=True)
    ...
    state = torch.load(f"models/pred_DMD_psl_{k_query}_{model_ver}.model",
                       map_location=device)
    ...
    df.to_excel(f'results/test_DMD_psl_{k_pole}_pole_top{top}.xlsx', index=False)
