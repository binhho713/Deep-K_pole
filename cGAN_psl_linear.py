...
class Generator(nn.Module):
    def __init__(self, noise_dim, condition_dim=171, output_dim=108, embedding_dim=171):
    ...
    def forward(self, noise, condition):
        # Embed condition and concatenate with noise
...
class Discriminator(nn.Module):
    def __init__(self, input_dim=108, condition_dim=171, embedding_dim=171, minibatch_features=10):
        super(Discriminator, self).__init__()
        self.embedding = nn.Embedding(condition_dim, embedding_dim)
        self.minibatch = MiniBatchDiscrimination(input_dim + embedding_dim, minibatch_features)

        self.model = nn.Sequential(
            nn.Linear(input_dim + embedding_dim + minibatch_features, 3072),
            nn.LeakyReLU(0.2),
            nn.Dropout(0.4),
    ...
    def forward(self, time_series, condition):
        # Embed condition and concatenate with time series
        y = self.embedding(condition)
...
# data
data = np.load("train_data/train_cGAN_psl.npy", allow_pickle=True)
